...
def evolve_population(population):
    ...
    parents = graded_population[
        :GRADED_INDIVIDUAL_RETAIN_COUNT
    ]  # de 0 à GRADED_INDIVIDUAL_RETAIN_COUNT-1

    # Randomly add other individuals to promote genetic diversity
    for individual in graded_population[
        GRADED_INDIVIDUAL_RETAIN_COUNT:
    ]:  # de GRADED_INDIVIDUAL_RETAIN_COUNT à la fin
    ...
    while len(children) < desired_len:
        ...
        if True:  # father != mother:
            child = (
                father[:MIDDLE_LENGTH_OF_EXPECTED_STR]
                + mother[MIDDLE_LENGTH_OF_EXPECTED_STR:]
            )
...
def main():
    ...
    while not solution and i < GENERATION_COUNT_MAX:
        ...
        if i & 255 == 255:
            print(
                "Current grade: %.2f" % average_grade,
                "/ %d" % MAXIMUM_FITNESS,
                "(%d generation)" % i,
            )
        if i & 31 == 31:
